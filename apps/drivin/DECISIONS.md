...
## The rocket jump (2026-09-08)
Stunts' "jump bug 3", where gravity went insane and threw the car so high that buildings looked like holes in the ground. The community's account is that the jump bugs and power gear both come out of the same handling of sloped surfaces and surface transitions at high speed (power gear is a truncation error in a 4-byte intermediate; the lead programmer confirmed it as a bug), and that it fired unpredictably when a fast car left a ramp, a loop or a seam. Ours needs the same three things: near top speed or speedlocked (`ROCKET_MIN_SPEED`), a launch off something sloped — a crest, a tilted surface or a jump lip, not a kerb (`ROCKET_SLOPE`) — and luck (`ROCKET_CHANCE`, a fifth of qualifying launches). The dice are a hash of the tick, so a run always rockets in the same places and replays stay honest. It leaves at `ROCKET_SPEED` straight up (about 290 m at the default) and, as in the original, a rocket flight can land from any height without wrecking. Settings → Experiments → Rocket jumps turns it off.

## Smooth ground, and hills you can actually jump (2026-09-08)
`sampleHeight` interpolates the corner grid as a Catmull-Rom surface, not bilinearly. Corners are 40 m apart, so bilinear made every cell a flat plane: a hill's crest fell on a cell boundary, the road draped over it had literally zero curvature, and the car stayed glued to the tarmac where it should have taken off. The spline still passes exactly through the corners, so the draped roads, the car's ground physics and the drawn mesh all agree. Two related fixes: a crest that lands on a seam between pieces now compares the slopes across the joint and launches when the road drops away faster than gravity can hold you; and "can I leave the road here" measures the road's height above the land rather than above the piece's level, so a hillside road no longer clamps your heading and steers you back onto it.

## Replays (2026-09-08)
Runs are recorded as poses (position, facing, wheels, speed) sampled at 30 Hz, not as inputs, so a saved replay plays back the same however the physics or the track change afterwards; each file carries a full copy of the track it was driven on, so watching it never depends on that track still existing or still looking the same. Files live on A:\ (localStorage, presented as a floppy: `A:\NAME.RPL`) and export to and import from real files. `I` or `F7` while driving watches the run so far; the pause and results menus offer Replay (watch, camera, save to A:\, browse), and the title menu has REPLAYS · A:\. While watching: space pauses, ← → scrub, ↑ ↓ change speed, C or 1–4 pick third person, first person, helicopter or TV camera, Esc returns where you came from. The helicopter holds station behind and above; the TV camera plants itself trackside, pans to follow, and cuts to a fresh spot once the car has run past. Saved key bindings are migrated on load (`keysV`) so aliases added later — P for pause — reach players who had already saved settings.
