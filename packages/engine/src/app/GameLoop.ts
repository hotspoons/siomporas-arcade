...
export class GameLoop {
  ...
  private readonly SIM_DT: number
  private readonly MAX_SUBSTEPS: number
  ...
  constructor(client: LoopClient, xrRenderer: { setAnimationLoop(fn: ((t: number) => void) | null): void } | null, opts: LoopOptions) {
    this.client = client
    this.renderer = xrRenderer
    this.SIM_DT = 1 / opts.simHz
    this.MAX_SUBSTEPS = opts.maxSubsteps
  }
  ...
  private readonly frame = (now: number): void => {
    ...
    if (frameDt > this.MAX_SUBSTEPS * this.SIM_DT) frameDt = this.MAX_SUBSTEPS * this.SIM_DT
    ...
    if (!this.paused) {
      ...
      while (this.accumulator >= this.SIM_DT && ticks < this.MAX_SUBSTEPS) {
        this.client.simTick(this.SIM_DT)
        this.accumulator -= this.SIM_DT
        ticks++
      }
      if (this.accumulator >= this.SIM_DT) this.accumulator = this.SIM_DT * 0.999 // dropped ticks, avoid spiral
      this.stats.simMs = performance.now() - simStart
    }
    this.stats.simTicks = ticks

    const alpha = this.accumulator / this.SIM_DT
    let presented = false
